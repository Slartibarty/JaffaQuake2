...
static void DrawGLPoly(glpoly_t* p)
{
	...
	for (; i < p->numverts; ++i, v += VERTEXSIZE)
	{
...
static void DrawGLFlowingPoly(msurface_t* fa)
{
	...
	for (; i < fa->polys->numverts; i++, v += VERTEXSIZE)
	{
	...
	if ( gl_dynamic->value )
	{
		...
		for ( surf = gl_lms.lightmap_surfaces[0]; surf != 0; surf = surf->lightmapchain )
		{
			...
			else
			{
				...
				if ( !LM_AllocBlock( smax, tmax, &surf->dlight_s, &surf->dlight_t ) )
				{
					ri.Sys_Errorf( ERR_FATAL, "Consecutive calls to LM_AllocBlock(%d,%d) failed (dynamic)\n", smax, tmax );
				}
...
#ifdef _WIN32
		//	R_SwapImage(smax, tmax, (byte*)temp);
#endif
...
static void R_DrawInlineBModel (void)
{
	...
	// draw the polygon
			...
			(!(psurf->flags & SURF_PLANEBACK) && (dot > BACKFACE_EPSILON)))
		{
			...
			else if ( gl_ext_multitexture->value && !( psurf->flags & SURF_DRAWTURB ) )
			{
...
// recurse down the children, front side first
	...
	for ( c = node->numsurfaces, surf = r_worldmodel->surfaces + node->firstsurface; c ; c--, surf++)
	{
		...
		{
			if ( gl_ext_multitexture->value && !( surf->flags & SURF_DRAWTURB ) )
			{
...
void GL_CreateSurfaceLightmap (msurface_t *surf)
{
	...
	if ( !LM_AllocBlock( smax, tmax, &surf->light_s, &surf->light_t ) )
	{
		...
		if ( !LM_AllocBlock( smax, tmax, &surf->light_s, &surf->light_t ) )
		{
			ri.Sys_Errorf( ERR_FATAL, "Consecutive calls to LM_AllocBlock(%d,%d) failed\n", smax, tmax );
		}
	}
...
//		glState.lightmap_textures	= gl_state.texture_extension_number;
//		glState.texture_extension_number = gl_state.lightmap_textures + MAX_LIGHTMAPS;
	}
