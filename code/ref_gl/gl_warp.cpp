...
static void SubdividePolygon (int numverts, float *verts)
{
	...
	if (numverts > 60)
		ri.Sys_Errorf (ERR_DROP, "numverts = %i", numverts);
	...
	for (i=0 ; i<3 ; i++)
	{
		...
		m = c_subdivide_size * floorf (m/c_subdivide_size + 0.5f);
		if (maxs[i] - m < 8)
			continue;
		if (m - mins[i] < 8)
			continue;
...
static constexpr float c_turbscale = (256.0f / (2.0f * M_PI));
...
constexpr const char *suf[6] = { "rt", "bk", "lf", "ft", "up", "dn" };
