...
void Mod_Modellist_f (void)
{
	...
	total = 0;
	ri.Con_Print (PRINT_ALL,"Loaded models:\n");
...
model_t *Mod_ForName (const char *name, qboolean crash)
{
	...
	if (i == mod_numknown)
	{
		if (mod_numknown == MAX_MOD_KNOWN)
			ri.Sys_Errorf (ERR_DROP, "mod_numknown == MAX_MOD_KNOWN");
		mod_numknown++;
	}
	...
	{
		if (crash)
			ri.Sys_Errorf (ERR_DROP, "Mod_NumForName: %s not found", mod->name);
	...
	default:
		ri.Sys_Errorf (ERR_DROP,"Mod_NumForName: unknown fileid for %s", mod->name);
		break;
	}
...
static void Mod_LoadVertexes (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		ri.Sys_Errorf (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
...
{	
	...
	if (count < 1 || count >= MAX_MAP_SURFEDGES)
		ri.Sys_Errorf (ERR_DROP, "MOD_LoadBmodel: bad surfedges count in %s: %i",
...
static void Mod_LoadBrushModel (model_t *mod, void *buffer)
{
	...
	if (i != BSPVERSION)
		ri.Sys_Errorf (ERR_DROP, "Mod_LoadBrushModel: %s has wrong version number (%i should be %i)", mod->name, i, BSPVERSION);
	...
	for (i=0 ; i<mod->numsubmodels ; i++)
	{
		...
		*starmod = *loadmodel;
		...
		if (starmod->firstnode >= loadmodel->numnodes)
			ri.Sys_Errorf (ERR_DROP, "Inline model %i has bad firstnode", i);
...
static void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	...
	if (version != ALIAS_VERSION)
		ri.Sys_Errorf (ERR_DROP, "%s has wrong version number (%i should be %i)",
...
//		ri.Sys_Errorf (ERR_DROP, "model %s has a skin taller than %d", mod->name,
//				   MAX_LBM_HEIGHT);

	if (pheader->num_xyz <= 0)
		ri.Sys_Errorf (ERR_DROP, "model %s has no vertices", mod->name);

	if (pheader->num_xyz > MAX_VERTS)
		ri.Sys_Errorf (ERR_DROP, "model %s has too many vertices", mod->name);

	if (pheader->num_st <= 0)
		ri.Sys_Errorf (ERR_DROP, "model %s has no st vertices", mod->name);

	if (pheader->num_tris <= 0)
		ri.Sys_Errorf (ERR_DROP, "model %s has no triangles", mod->name);

	if (pheader->num_frames <= 0)
		ri.Sys_Errorf (ERR_DROP, "model %s has no frames", mod->name);
...
static void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	if (sprout->numframes > MAX_MD2SKINS)
		ri.Sys_Errorf (ERR_DROP, "%s has too many frames (%i > %i)",
