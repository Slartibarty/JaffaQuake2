...
===============
*/
void PF_cprint (edict_t *ent, int level, const char *msg)
{
	int n;
	...
	if (ent)
		SV_ClientPrint (svs.clients+(n-1), level, msg);
	else
		Com_Print (msg);
}
...
===============
*/
void PF_centerprint (edict_t *ent, const char *msg)
{
	int n;
...
{
	Com_Errorf (ERR_DROP, "Game Error: %s", msg);
}
...
void PF_Configstring (int index, const char *val)
{
	if (index < 0 || index >= MAX_CONFIGSTRINGS)
		Com_Errorf (ERR_DROP, "configstring: bad index %i\n", index);
...
void SV_InitGameProgs (void)
{
	...
	if (ge->apiversion != GAME_API_VERSION)
		Com_Errorf (ERR_DROP, "game is version %i, not %i", ge->apiversion,
		GAME_API_VERSION);
