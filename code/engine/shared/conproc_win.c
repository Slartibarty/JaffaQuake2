...
#if id386
#define atoifunc atoi
#else
#define atoifunc atoll
#endif
...
// give QHOST a chance to hook into the console
	...
	{
		if (t < argc)
			hFile = (HANDLE)atoifunc (ccom_argv[t+1]);
	}
		
	if ((t = CCheckParm ("-HPARENT")) > 0)
	{
		if (t < argc)
			heventParent = (HANDLE)atoifunc (ccom_argv[t+1]);
	}
		
	if ((t = CCheckParm ("-HCHILD")) > 0)
	{
		if (t < argc)
			heventChild = (HANDLE)atoifunc (ccom_argv[t+1]);
	}
