...
*/
// engine.c -- misc functions used in client and server
...
=============
*/
void Com_Error (int code, const char *msg)
{	
...
=============
*/
void Com_Errorf (int code, const char *fmt, ...)
{
	va_list		argptr;
	char		msg[MAXPRINTMSG];
...
void *SZ_GetSpace (sizebuf_t *buf, int length)
{
	...
	if (buf->cursize + length > buf->maxsize)
	{
		...
		if (length > buf->maxsize)
			Com_Errorf (ERR_FATAL, "SZ_GetSpace: %i is > full buffer size", length);
...
void *Z_TagMalloc (int size, int tag)
{
	...
	z = malloc(size);
	if (!z)
		Com_Errorf (ERR_FATAL, "Z_Malloc: failed on allocation of %i bytes",size);
	memset (z, 0, size);
	z_count++;
	z_bytes += size;
	z->magic = Z_MAGIC;
...
void Engine_Init (int argc, char **argv)
{
	...
	timescale = Cvar_Get ("timescale", "0", 0);
