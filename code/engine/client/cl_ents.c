...
void CL_ParsePacketEntities (frame_t *oldframe, frame_t *newframe)
{
	...
	while (1)
	{
		...
		if (newnum >= MAX_EDICTS)
			Com_Errorf (ERR_DROP,"CL_ParsePacketEntities: bad number:%i", newnum);
...
//PGM
			...
			else if (effects & EF_BFG)
			{
				...
				V_AddLight (ent.origin, i, 0.0f, 1.0f, 0.0f);
			}
			// RAFAEL
			...
			else if (effects & EF_FLAG1)
			{
				...
				V_AddLight (ent.origin, 225.0f, 1.0f, 0.1f, 0.1f);
			}
			else if (effects & EF_FLAG2)
			{
				CL_FlagTrail (cent->lerp_origin, ent.origin, 115.0f);
				V_AddLight (ent.origin, 225.0f, 0.1f, 0.1f, 1.0f);
			}
//======
//ROGUE
			else if (effects & EF_TAGTRAIL)
			{
				...
				V_AddLight (ent.origin, 225.0f, 1.0f, 1.0f, 0.0f);
			}
