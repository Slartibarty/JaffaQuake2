...
//		"[Rendition OpenGL]",
	...
	s_screensize_slider[SOFTWARE_MENU].curvalue = scr_viewsize->value/10.0f;
	s_screensize_slider[OPENGL_MENU].curvalue = scr_viewsize->value/10.0f;
	...
	s_software_menu.x = viddef.width * 0.5f;
	s_software_menu.nitems = 0;
	s_opengl_menu.x = viddef.width * 0.5f;
	...
	for ( i = 0; i < 2; i++ )
	{
		...
		s_brightness_slider[i].minvalue = 5.0f;
		s_brightness_slider[i].maxvalue = 13.0f;
		s_brightness_slider[i].curvalue = ( 1.3f - vid_gamma->value + 0.5f ) * 10.0f;
	...
	Menu_AddItem( &s_software_menu, ( void * ) &s_mode_list );
