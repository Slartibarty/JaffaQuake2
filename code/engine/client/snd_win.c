	...
	if (!s_wavonly->value)
	{
		if (snd_firsttime || snd_isdirect)
		{
			...
			if (stat == SIS_SUCCESS)
			{
				...
				if (snd_firsttime)
					Com_Print ("dsound init succeeded\n" );
			}
			else
			{
				snd_isdirect = false;
				Com_Print ("*** dsound init failed ***\n");
			}
		}
	}
...
// to have sound)
	if (!dsound_init && (stat != SIS_NOTAVAIL))
	{
		if (snd_firsttime || snd_iswave)
		{
			...
			{
				if (snd_firsttime)
					Com_Print ("Wave sound init succeeded\n");
			}
			else
			{
				Com_Print ("Wave sound init failed\n");
			}
		}
	}
	...
	{
		if (snd_firsttime)
			Com_Print ("*** No sound device initialized ***\n");
...
//Com_Printf ("completed %i\n", snd_completed);
	...
	while (((snd_sent - snd_completed) >> sample16) < 8)
	{
		...
		{ 
			Com_Print ("Failed to write block to device\n");
			FreeSound ();
			return; 
		} 
	}
