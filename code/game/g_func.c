...
again:
	if (!self->target)
	{
//		gi.dprint ("train_next: no next target\n");
		return;
	}
...
void func_train_find (edict_t *self)
{
	...
	if (!self->target)
	{
		gi.dprint ("train_find: no target\n");
		return;
	}
	ent = G_PickTarget (self->target);
	if (!ent)
...
void trigger_elevator_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	{
//		gi.dprint("elevator busy\n");
		return;
	}

	if (!other->pathtarget)
	{
		gi.dprint("elevator used with no pathtarget\n");
		return;
	}
...
void trigger_elevator_init (edict_t *self)
{
	if (!self->target)
	{
		gi.dprint("trigger_elevator has no target\n");
		return;
	}
