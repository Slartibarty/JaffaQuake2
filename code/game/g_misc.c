...
void SP_func_wall (edict_t *self)
{
	...
	{
//		gi.dprint("func_wall missing TRIGGER_SPAWN\n");
		self->spawnflags |= 1;
	}
	...
	if (self->spawnflags & 4)
	{
		if (!(self->spawnflags & 2))
		{
			gi.dprint("func_wall START_ON without TOGGLE\n");
			self->spawnflags |= 2;
		}
	}
...
void teleporter_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (!dest)
	{
		gi.dprint ("Couldn't find destination\n");
		return;
	}
...
void SP_misc_teleporter (edict_t *ent)
{
	...
	if (!ent->target)
	{
		gi.dprint ("teleporter without a target.\n");
		G_FreeEdict (ent);
		return;
	}
