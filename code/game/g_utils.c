...
edict_t *G_PickTarget (char *targetname)
{
	...
	if (!targetname)
	{
		gi.dprint("G_PickTarget called with NULL targetname\n");
		return NULL;
	}
...
//
	...
	// create a temp object to fire at a later time
		...
		if (!activator)
			gi.dprint ("Think_Delay with no activator\n");
...
//
	...
	{
		gi.centerprint (activator, ent->message);
		if (ent->noise_index)
...
//
	if (ent->killtarget)
	{
		...
		while ((t = G_Find (t, FOFS(targetname), ent->killtarget)))
		{
			...
			if (!ent->inuse)
			{
				gi.dprint("entity was removed while using killtargets\n");
				return;
			}
		}
	}
