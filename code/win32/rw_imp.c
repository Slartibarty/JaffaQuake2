...
void VID_CreateWindow( int width, int height, int stylebits )
{
	...
	// slarthack: fullscreen fix
	ri.Cvar_SetValue ("vid_fullscreen", 0.0f);
...
================
*/
void Sys_MakeCodeWriteable (void *startaddr, size_t length)
{
	DWORD  flOldProtect;

	if (!VirtualProtect (startaddr, length, PAGE_READWRITE, &flOldProtect))
